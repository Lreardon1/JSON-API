...
internal func exerciseOne() {
    ...
    let locationDictionary = userData["results"][0]["location"]
    let streetName = locationDictionary["street"].stringValue
    let cityName = locationDictionary["city"].stringValue
    let stateName = locationDictionary["state"].stringValue
    ...
    print("\(firstName) \(lastName) lives at \(streetName) in \(cityName), \(stateName), \(postCode). If you want to contact \(title). \(lastName), you can email \(emailAddress) or call at \(phoneNumber).")
...
internal func exerciseThree() {
    ...
    for i in 0...24 {
        allMovies.append(Movie(json: allMoviesData[i]))
    }
